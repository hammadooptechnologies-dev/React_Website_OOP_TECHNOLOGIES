...
const PortfolioTabs = () => {
  ...
  {/* Tabs Container */}
  <div className="flex flex-wrap justify-center sm:justify-between items-center border border-gray-300 rounded-full max-w-[1140px] mx-auto gap-2 sm:gap-3 p-2">
    {tabs.map((tab) => (
      <button
        key={tab}
        onClick={() => setActiveTab(tab)}
        className={`px-3 sm:px-4 py-2 uppercase text-sm md:text-[14px] rounded-full transition-all duration-300 ${
          activeTab === tab
